  ...
  <!-- Profile Image -->
    <div class="box box-primary">
      <div class="box-body box-profile">
        ...
        {{--
          <!-- <form action="{{ route('ChangeAssignTo',$client->id)}}" method="post">
            {{ csrf_field() }}

            <div class="form-group">
              <label for="exampleInputEmail1">Assign to</label>
              <select class="js-select2 form-control" name="parent">
                @foreach($parent as $data)
                  @if($data->name == $client->parent)

                    <option  value="{{$data->id}}" selected >{{$client->parent}}</option>
                  @endif
            ...
            <div class="form-group">
              <input type="submit" class="form-control btn btn-success" value="Submit Assign To">
            </div>
          </form> -->
